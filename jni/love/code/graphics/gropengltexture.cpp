...
#else
			if ( !reload ) {
				glTexImage2D (GL_TEXTURE_2D, 0, GL_RGBA, mipmap_w, mipmap_h, 0, GL_RGBA, GL_UNSIGNED_SHORT_5_5_5_1, (resize) ? texmem : bmp_data);
			} else {
				glTexSubImage2D (GL_TEXTURE_2D, 0, 0, 0, mipmap_w, mipmap_h, GL_RGBA, GL_UNSIGNED_SHORT_5_5_5_1, (resize) ? texmem : bmp_data);
			}
#endif
